...
Issue (이미지 첨부) → blog-draft.yml workflow trigger
  ...
  → flows/blog-draft/scripts/draft.mjs          (스타일 + 트렌드 + 사진 + imageCount → 채널별 초안)
  → gh issue comment                            (트렌드 메타 + 채널별 초안 한 코멘트)
```
...
- GitHub repo secrets:
  ...
  - `ATTACHMENTS_PAT` — **private 레포에서만 필요**. user-attachments 자산 접근용 classic PAT(`ghp_...`, `repo` scope). [토큰 생성](https://github.com/settings/tokens/new?scopes=repo&description=culcom-routines%20attachments). fine-grained PAT(`github_pat_...`)는 user-attachments에서 거부될 수 있으니 classic 권장.
- Issue 라벨 (사전 생성 권장):
